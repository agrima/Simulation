...
TABLE tbl, tbl2;			/*pointer for table */
...
void sim()					/*1st process - named sim */
{
	...
	for(j = 0; j < NRUNS; ++j) {
		...
		tbl2 = table("service times");		/*initialize table */
		...
		printf("%lf\t%lf\t%lf\n", RHO, table_mean(tbl), table_mean(tbl2));
...
{
	double t1, sum, service_time, sum_service_time;
	int i;
 
	create("cust");					/*required create statement*/

	sum = 0.0; sum_service_time = 0.0;
	for(i = 0; i < NSERVERS; ++i){
		t1 = clock;					/*time of request */
		reserve(f[i]);				/*reserve facility f*/
		sum += (clock-t1);
		service_time = exponx(&seed2, SVTM);
		hold(service_time);	/*hold service time*/
		sum_service_time += service_time;
		release(f[i]);				/*release facility f*/
	}
	record(sum, tbl);
	record(sum_service_time, tbl2);
